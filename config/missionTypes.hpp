...
class helpCivWater : base_type //task invio acqua ai cittadini.
{
	scope			= 1;
	name			= "helpWater";

	taskShort		= "$STR_SMEGen_type_helpCiv_shrt";
	task			= "$STR_SMEGen_type_helpCiv_task";
	description		= "$STR_SMEGen_type_helpCiv_desc";

	class conditions
	{
		class win
		{
			condition	= "(({ _x iskindof 'Land_WaterBottle_01_stack_F' } count (( getMarkerPos '#__MARKER_NAME__#' ) nearEntities #__MARKER_SIZE__# )) > 1 )";
			function	= "BIS_fnc_taskSetState";
			isFinal		= 1;
		};
	};
	class groups : groups
	{
		class group01 : base_fireteam { scope = 1; task = "PATROL_AROUND"; };
	};
};
//fine task
